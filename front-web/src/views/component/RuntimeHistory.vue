...
import {defaultOption, doubleSeries, singleSeries} from "@/echart";
...
const localTimeLine = (list:any) => list.map((item:any) => new Date(item.timestamp).toLocaleString())

function updateCpuUsage(list:any) {
  const chart = charts[0]
  let data = list.map((item:any) => (item.cpuUsage  * 100).toFixed(1))
  const option = defaultOption('CPU(%)', localTimeLine(list))
  singleSeries(option, 'CPU使用率(%)', data, ['#72c4fe', '#72d5fe', '#2b6fd733'])
  chart.setOption(option)
}

function updateMemoryUsage(list:any) {
  const chart = charts[1]
  let data = list.map((item:any) => (item.memoryUsage * 1024).toFixed(1));
  const option = defaultOption('内存(MB)', localTimeLine(list))
  singleSeries(option, '内存使用(MB)', data, ['#6be3a3', '#bbfad4', '#A5FFD033'])
  chart.setOption(option);
}

function updateNetworkUsage(list:any) {
  const chart = charts[2]
  let data = [
    list.map((item:any) => item.networkUpload),
    list.map((item:any) => item.networkDownload)
  ]
...
function updateDiskUsage(list:any) {
  const chart = charts[3]
  let data = [
    list.map((item:any) => item.diskRead.toFixed(1)),
    list.map((item:any) => item.diskWrite.toFixed(1))
  ]
